...
public class MainActivity extends Activity implements Constants, MeetingServiceListener, ZoomSDKInitializeListener {
	...
	private void selectTab(int tabId) {
		if(tabId == TAB_WELCOME) {
			...
			ZoomSDK zoomSDK = ZoomSDK.getInstance();
			if(!zoomSDK.isInitialized()) {
				Toast.makeText(this, "TnqGuru has not been initialized successfully", Toast.LENGTH_LONG).show();
				return;
			}
	...
	public void onZoomSDKInitializeResult(int errorCode, int internalErrorCode) {
		...
		if(errorCode != ZoomError.ZOOM_ERROR_SUCCESS) {
			Toast.makeText(this, "Failed to initialize TnqGuru. Error: " + errorCode + ", internalErrorCode=" + internalErrorCode, Toast.LENGTH_LONG);
		} else {
			Toast.makeText(this, "Initialize TnqGuru successfully.", Toast.LENGTH_LONG).show();
